...
- Phase 2 ④ 🚧 `scripts/trot_demo.py`：站立（stand 步态）长期稳定；原地踏步（walk 步态）约 3.8s / 5 个周期后翻倒。
  ...
  支撑腿并没有分担。候选(a)（MPC 预测式主镇定）、候选(4)(5)（两版负载均摊正则）先后都证伪，五个候选全部
  指向同一个结论：单层加权和 QP 没有任务优先级保证。**改成真正的任务优先级（分层）QP 后突破**：
  `WholeBodyController.solve_hierarchical()`（姿态+高度 > 支撑腿零滑移+摆动腿跟踪 > 力跟踪，三级依次求解，
  低优先级不能牺牲高优先级已达到的精度）——walk 原地踏步从"必在 3.82s 摔倒"变成**稳定撑满 10s 未摔倒**
  （`max|roll|=54.4°` 但从未越过 60° 摔倒阈值，`rms roll≈37°` 说明还在明显晃、不是干净的稳定，但是第一个
  真正跑赢基线的方案）。trot（period=0.4）和有前进速度的 walk 还没跟上（分别 1.73s / 1.67s 摔），
  且 `wbc_fail` 仍偏高（~100+/5000 拍，锁定约束的容差 `eps_*` 还没调好），需要继续打磨。详见 Phase 2④
  最新记录。诊断图 `results/transition_diagnostic.png`、`results/stance_overlap_diagnostic.png`、
