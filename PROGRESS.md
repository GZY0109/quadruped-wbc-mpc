...
- Phase 2 ④ 🚧 `scripts/trot_demo.py`：站立（stand 步态）长期稳定；原地踏步（walk 步态）约 3.8s / 5 个周期后翻倒。
  ...
  "调弱姿态增益"直觉修法被证伪；**接着测的"支撑相重叠"修法（候选(b)）也被证伪，且方向相反**——延迟下一条
  腿抬起、制造四足过渡缓冲窗口，两种严格控制混淆变量的实现都让存活时长从 3.82s 降到 1.6~1.8s，饱和时刻从
  38ms 提前到 12~20ms。真正的杠杆还没定位到，候选方向 (a)(c) 待选，见下方"待决策"。诊断图
  `results/transition_diagnostic.png`、`results/stance_overlap_diagnostic.png`，两段真实视频
  `results/stand_demo.mp4` / `results/walk_demo.mp4`。
...
- **[待决策 - Phase 2④ trot 镇定，下一步方向] 深挖"支撑相重叠为何更差"已经排除了三个候选解释**
  （摆动轨迹本身、落脚点目标、触地时基座状态都跟基线几乎一致），把问题精确缩小到：**QP 求解出的姿态
  纠偏力矩几乎 100% 只流向刚建立接触的那条腿，其余支撑腿的动态力矩全程贴 0，不管有几条腿在支撑**。
  这直接推翻了候选 (c)（"分摊给其余支撑腿"）的前提假设本身也不成立——除非先搞清楚 QP 为什么不分摊。
  候选 (a)（MPC 预测式主镇定）没有这个前提问题，依然可选。详见 Phase 2④ 最新记录的完整数据和图
  （`results/stance_overlap_deep_dive.png`）。**需要用户拍板**：继续深挖"QP 为什么把力矩集中在新接触腿
  上"这个更底层的机制（比如 MuJoCo 新建立接触的柔顺爬升、还是 WBC 软任务权重的结构性偏置），还是直接
  转去做候选 (a)。
