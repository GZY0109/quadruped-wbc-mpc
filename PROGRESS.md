...
`Phase 2 - 进行中（①②③ 完成；④ 集成回路搭好，站立稳，walk/trot 调参中——已定位关键突破）`
...
- Phase 2 ④ 🚧 `scripts/trot_demo.py`：gait+MPC+WBC(500Hz)+摆动腿闭环已搭好。
  **用默认 `WBCGains()` 时闭环站立（含 stand/所有腿支撑步态）非常稳**（roll≈0、h=27cm 保持）。
  **关键突破**：之前 walk/trot 一直摔，最后定位到——是我为"调稳"而覆盖的那套增益本身把 baseline 搞坏了；
  用回默认增益后站立恢复稳定。**下一步立刻要做的**：用默认 `WBCGains()` 重测 walk（静稳，3 足支撑）与 trot。
  详见下方记录里的完整排查链路与下次接续指令。
...
- **[已决策 - Phase 2④ trot 镇定方向] 采用方案 (a)：MPC 做预测式主镇定 + WBC 忠实跟踪 MPC 反力。**
  ...
  （踩坑教训见 Phase 2④ 记录：不要再盲目动基座增益，默认增益站立是稳的。）
