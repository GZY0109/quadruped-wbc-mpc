...
`Phase 2 - 进行中（①②③ 完成；④ 集成回路两个真实 bug 已修复，站立长期稳定，原地踏步能撑约 3.8s / 5 个步态周期后翻倒）`
...
- Phase 2 ④ 🚧 `scripts/trot_demo.py`：这一轮定位并修复了两个真实 bug（摆动轨迹加速度不连续、
  摆动任务能"劫持"基座加速度），把原地踏步（walk 步态）的存活时间从 <1.5s 提升到约 3.8s（近 5 个步态周期）。
  **站立（stand 步态）用默认增益长期稳定**（roll≈0、h=27cm 不变）；**walk/trot 连续行走仍会在数个周期后翻倒**，
  根因是单一加权 QP 的 WBC 对多接触切换调参很敏感（详见记录）。已出两段真实视频：
  `results/stand_demo.mp4`（长期稳定）、`results/walk_demo.mp4`（诚实展示：踏步到翻倒全过程）。
  **下一步**：往任务优先级/分层 QP（hard task hierarchy）方向改 WBC，而不是继续调权重数字。
