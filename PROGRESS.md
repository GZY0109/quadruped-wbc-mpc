...
`Phase 2 - 进行中（①②③ 完成；④ 发现单次确定性跑测不可靠，改为多次扰动统计方法论后，"分层QP解决walk"
的结论被部分推翻——分层QP确实比单层QP好，但不是"解决"，三个场景现在处于同一量级、需要新的共同突破口）`
...
- Phase 2 ④ 🚧 `scripts/trot_demo.py`：站立（stand 步态）长期稳定。翻倒根因链条（力矩饱和机制的精细诊断、
  五个候选修法证伪、最终改成任务优先级分层 QP）见下方历史记录，结论仍然有效。**但"walk 原地踏步分层QP后
  稳定撑满10s"这个具体数字已被推翻**——见最新记录：新会话复现时发现同一份代码在环境重装（未锁定的 pip
  依赖版本漂移）后，三个基准场景的单次跑测结果全部发生了 2~4 秒量级的变化，说明控制器处于真正的边际稳定
  状态，对浮点数值噪声敏感，单次确定性跑测（`init_noise_seed=None`）不能作为判断"参数是否有效"的可靠依据。
  改为多次扰动统计方法论（`run_trials()`，n=5 个种子的初始关节噪声）后的诚实结果：**分层QP相对单层QP的
  改进方向是真实且可复现的**（同一组 walk 基线，单层 0.87±0.10s 全灭 vs 分层 2.69±1.31s 全灭，两者的
  存活时间分布几乎不重叠，改进有统计意义），**但"稳定不摔"没有实现**——walk 原地/trot/前进walk 三个场景
  在扰动下 100% 会摔，均值都在 2.6~3.0s 量级，不再是"walk 已解决、trot/前进walk 落后"的格局，而是三者
  处于同一水平、还没有找到能让分层QP真正"稳"（而不只是"晚摔"）的下一个突破口。
  诊断图 `results/transition_diagnostic.png`、`results/stance_overlap_diagnostic.png`、
  `results/stance_overlap_deep_dive.png`、`results/hierarchical_vs_flat.png`（诚实的多次扰动对比），
  视频 `results/stand_demo.mp4` / `results/walk_demo.mp4` / `results/walk_hierarchical_demo.mp4`
  （零噪声巧合跑，仅存档）/ `results/walk_hierarchical_representative.mp4`（存活时间最接近均值的代表性跑）。
  环境装错的疑虑已核实并排除（确定性组件自检全部精确匹配历史数字）。快速诊断进一步定位到：分层QP在纯站立
  遇到小扰动时反而比单层QP更容易发散（Stage 1 命令角加速度在clip上限反复震荡），但"给Stage 1补正则化"
  这个具体修法已测试并证伪（改动已还原），真正根因还需要新的假说。是否继续投入见"待决策"。
...
**这一节已过时被推翻，见最新记录（"复现失败→发现依赖版本漂移→改多次试验方法论"）**——`walk` 原地踏步
"稳定撑满10s"不是可靠结论，`eps_*` 针对 trot/前进walk 重调这个计划的前提已经不成立（三个场景现在处于
同一水平，不是"walk 解决、trot 落后"）。留着这段文字只为存档排查过程，下次会话直接从最新记录接着做：

1. `bash scripts/setup_env.sh && python scripts/stand_test.py` 确认环境；`requirements.txt` 已锁定精确
   版本（mujoco==3.13.0 / osqp==1.1.3 / numpy==2.5.3 / scipy==1.18.1），装出来的版本应该完全一致。
2. 用 `scripts/trot_demo.py` 的 `run_trials(n=5, ...)` 复现最新记录里的三组统计基准（flat QP 0.87±0.10s、
   分层QP walk/trot/前进walk 均 2.6~3.0s 量级、100% fall_rate），确认环境对。
3. 从"为什么分层QP能让存活时间从 0.87s 涨到 2.6~3.0s，但涨了之后还是会摔"这个新问题继续挖，而不是回到
   "针对 trot 单独调 eps"这个已经过时的计划——**待用户和你一起确认下一步方向**。

<details><summary>旧计划原文（已过时，仅存档）</summary>
...
- **[待决策 - Phase 2④ 是否继续投入 / 转 Phase 3]** 分层QP相对单层QP的改进有统计意义（0.87s→2.6~3.0s），
  但三个步态场景扰动下 100% 会摔。快速诊断定位到一个具体现象（分层QP在纯站立扰动下比单层QP更脆弱、
  Stage 1 命令角加速度在clip上限反复震荡）但"补正则化"这个对应修法被证伪，真正根因需要新的、目前还没有
  的假说。用户尚未拍板：继续投入新一轮诊断，还是就此打住转 Phase 3（并把 Phase 3/简历叙事从"实现稳定
  行走"改写为"分层QP vs 单层QP 的抗扰动量化对比"）。详见 Phase 2④ 最新两条记录。
