...
- **[待决策 - Phase 2④ trot 镇定，策略层面重新讨论] 候选 (a)(b)(c的朴素前提)(4-负载均摊初版) 四个方向
  都已经用真实数据证伪**，指向的是同一类根因——反应式 WBC（每 2ms 单独解一个加权和 QP，没有任务优先级/
  没有时域预测/没有跨腿的载荷比较机制）在接触构型不够冗余的瞬间镇不住这个边际稳定系统，MPC 又因为
  30Hz/300ms horizon 的带宽上限帮不上快速修正的忙。详见下方 Phase 2④ 最新记录。**需要跟用户重新讨论
  下一步策略**（不是继续在同一个反应式 WBC 框架里找下一个候选修法）：比如认真做一次任务优先级/分层 QP
  （之前的尝试可能实现得不够到位）、把负载均摊做成"拉向几何/质心算出的合理份额"而不是"拉向均值"、
  还是收缩 Phase 3 目标（交付"稳定站立+抗扰恢复"而不是动态 walk/trot）、还是别的方向。
