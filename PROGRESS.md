...
- **[待决策 - Phase 2④ trot 镇定，下一步方向] 候选 (b)"支撑相重叠缓冲"已用两种严格隔离混淆变量的实现
  测试并证伪，且是反方向的证伪（不是"无效"，是"更差"）。候选 (a)（MPC 预测式主镇定，逐步把 MPC 力权重
  接进 WBC）和候选 (c)（把纠偏力矩从单条落地腿分摊到其余支撑腿，需要动 WBC 任务分配）都还没试。
  详见下方 Phase 2④ 最新记录的完整数据和图。**需要用户拍板下一步选 (a) 还是 (c)**，或者先深挖一下
  "为什么支撑相重叠会让情况变差"这个本身违反直觉的结果（可能本身就是有信息量的诊断线索）。
