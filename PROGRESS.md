...
- Phase 2 ④ 🚧 `scripts/trot_demo.py`：站立（stand 步态）长期稳定。翻倒根因链条（力矩饱和机制的精细诊断、
  ...
  `results/stance_overlap_deep_dive.png`，视频 `results/stand_demo.mp4` / `results/walk_demo.mp4` /
  `results/walk_hierarchical_demo.mp4`（这段视频对应的是零噪声下的巧合稳定跑，不代表统计意义上的典型结果，
  待补充能反映真实分布的新素材）。
